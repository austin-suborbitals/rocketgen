...
struct Chamber {
    ...
    // TODO: express m^2 in return type
    auto flatwall_lateral_area() const { return 2 * M_PI * flatwall_radius * flatwall_length; }
    ...
    quantity<length_d> converging_slant_height() const {
        return sqrt(nth_power<2>(flatwall_radius-throat_radius) + nth_power<2>(converging_length));
    }

    // TODO: express m^2 in return type
    auto converging_lateral_area() const {
        return (flatwall_radius + throat_radius) * M_PI * converging_slant_height();
    }
    ...
    // TODO: express m^2 in return type
    auto surface_area() const { return flatwall_lateral_area() + converging_lateral_area(); }
...
Chamber build_chamber(latex::doc::Report& doc, const EngineBasis& rocket, const Throat& throat) {
    ...
    section
        ...
        << (Subsection("Summary")
            << vspace
            << (UnorderedList()
                << strjoin(PrefixText("Flatwall Section Diameter:"),    scale_to_string(diameter, 1, mm, "mm"))
                << strjoin(PrefixText("Flatwall Section Area:"),        scale_to_string(area, 1, mm_sq, "mm+2"))
                << strjoin(PrefixText("Flatwall Section Length:"),      scale_to_string(length-converging_length, 1, mm, "mm"))
                << strjoin(PrefixText("Flatwall Lateral Area:"),        scale_to_string(chamber.flatwall_lateral_area(), 1, cm_sq, "cm+2"))
            ...
            << (UnorderedList()
                ...
                << strjoin(PrefixText("Converging Lateral Area:"),      scale_to_string(chamber.converging_lateral_area(), 1, cm_sq, "cm+2"))
            ...
            << svspace
            << (UnorderedList()
                << strjoin(PrefixText("Total Chamber Length:"),         scale_to_string(length, 1, mm, "mm"))
                << strjoin(PrefixText("Total Surface Area:"),           scale_to_string(chamber.surface_area(), 1, cm_sq, "cm+2"))
